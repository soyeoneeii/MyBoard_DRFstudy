...
from rest_framework.authtoken.models import Token #Token 모델 사용할것
from rest_framework.validators import UniqueValidator #이메일 중복 방지를 위한 검증 도구

from django.contrib.auth import authenticate
#장고의 기본 authenticate 함수. 우리가 설정한 DefaultAuthBackend인 TokenAuth 방식으로 유저를 인증해줌
...
class RegisterSerializer(serializers.ModelSerializer): #회원가입 시리얼라이저
    ...
    def create(self,validated_data):        #CREATE 요청에 대해 create메소드를 오버라이딩, 유저를 생성하고 토큰을 생성하게 함
